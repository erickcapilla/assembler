...
public class Tokens {
  ...
  public void getToken() {
    ...
    for (String line : this.lines) {
      ...
      for (int i = 0; i < this.splitLine.size(); i++) {
        if(this.splitLine.get(i).contains(" ") || this.splitLine.isEmpty()) {
  ...
  public void writeFile() {
    try {
      ...
      this.tokens.forEach(t -> writer.println(Arrays.toString(t).replace(":", "") +
                          ". Hay " + t.length + " tokens"));
      file.close();
    } catch (Exception e) { e.printStackTrace(); }
  }
  ...
  public void deleteDuplicate() {
    ...
    /*Set<String> hashSet = new HashSet<>(this.splitLine);
    this.splitLine.clear();
    this.splitLine.addAll(hashSet);*/
  }
